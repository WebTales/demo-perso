...
class SearchResource extends AbstractResource
{
    ...
    public function __construct()
    {
        ...
        $this->searchParamsArray = array('orderby', 'orderbyDirection', 'query', 'objectType', 'type', 'damType', 'userType', 'author',
            'userName', 'lastupdatetime','textfl','start', 'limit', 'searchMode','price','inStock',"isMagic","fingerprint","historyDepth","historySize","useDetailContent","detailContentId");
        $this
            ->definition
            ->setName('Search')
            ...
            ->editVerb('get', function (VerbDefinitionEntity &$entity) {
                $entity
                    ...
                    ->addInputFilter(
                        (new FilterDefinitionEntity())
                            ->setKey('includeTerms')
                            ->setDescription('Include term labels')
                    )
                    ->addInputFilter(
                        (new FilterDefinitionEntity())
                            ->setKey('type')
                        ...
                        (new FilterDefinitionEntity())
                            ->setKey('textfl')
                            ->setDescription('Content first letter')
                    )
                    ->addInputFilter(
                        (new FilterDefinitionEntity())
                            ->setKey('damType')
        ...
        foreach ($results['data'] as $key => $value) {
            switch ($value['objectType']) {
                ...
                case 'content':
                    ...
                    if (isset($params["includeTerms"])){
                        $results['data'][$key]["taxoLabels"]=[];
                        foreach($results['data'][$key] as $dataKey => $dataValue){
                            if (strpos($dataKey,"taxonomy.")!==false && $dataKey !== "taxonomy.navigation" && is_array($dataValue)){
