angular.module("rubedoBlocks").lazy.controller("RCCarrouselController",["$scope","$location","RubedoSearchService","$route","RubedoPagesService",
    function($scope,$location,RubedoSearchService,$route,RubedoPagesService){
        var me = this;
        ...
        if (config&&config.rcPage&&mongoIdRegex.test(config.rcPage)){
            RubedoPagesService.getPageById(config.rcPage).then(function(response){
                if (response.data.success){
